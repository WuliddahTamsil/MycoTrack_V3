...
import { Wallet, ArrowUpDown, ArrowDown, Plus, Loader2 } from 'lucide-react';
...
export const FarmerBalance: React.FC = () => {
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/user/transactions?userId=${user?.id}&userType=farmer`);
      if (response.ok) {
  ...
  const getTypeLabel = (transaction: Transaction) => {
    if (transaction.type === 'credit') {
      ...
      if (transaction.description?.includes('pesanan') || transaction.description?.includes('Pembayaran')) {
        return 'Penjualan';
      }
      return 'Kredit';
    } else {
