...
import { Wallet, ArrowUpDown, ArrowDown, Plus } from 'lucide-react';
import { mockTransactions } from '../mockData';
import { useAuth } from '../AuthContext';
import { toast } from 'sonner@2.0.3';

export const FarmerBalance: React.FC = () => {
  const { user, updateBalance } = useAuth();
  const [transactions] = useState(mockTransactions.filter(t => t.userId === user?.id || t.userId === 'f1'));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-[#B82601] font-bold">Saldo & Keuangan</h2>
        <p className="text-[#5A4A32] font-medium">
          Kelola saldo dan riwayat transaksi keuangan Anda
        </p>
      </div>
                  ...
                  <TableRow key={transaction.id} className="border-[#FF7A00]/10">
                    ...
                    <TableCell>
                      <span className={`px-3 py-1 rounded-full text-xs font-bold text-white ${
                        transaction.amount > 0 ? 'gradient-orange-warm' : 'gradient-red-fire'
                      }`}>
                        {getTypeLabel(transaction.type)}
                      </span>
                    </TableCell>
                    <TableCell className="text-[#2D2416] font-medium">{transaction.description}</TableCell>
                    <TableCell className={`font-bold ${transaction.amount > 0 ? 'text-[#9A7400]' : 'text-[#B82601]'}`}>
