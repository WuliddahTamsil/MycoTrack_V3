...
import { Sparkles, Shell, ArrowLeft, Mail, Lock, User } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-[#FFF8F0] via-[#FFE4CC] to-[#FFF4D4]">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-gradient-to-br from-[#FF7A00]/5 to-[#E8A600]/5 rounded-full blur-3xl animate-pulse-soft" style={{ animationDelay: '1s' }}></div>
      ...
      <div className={`relative z-10 w-full max-w-md mx-auto px-4 transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        <div className="autumn-card border-[#FF7A00]/10 p-8 autumn-shadow-lg backdrop-blur-sm bg-white/95 hover:shadow-2xl transition-all duration-300 transform hover:scale-[1.02]">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gradient-autumn mb-2 animate-fade-in">Selamat Datang</h1>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="animate-slide-in-up" style={{ animationDelay: '0.3s' }}>
              <Label htmlFor="role" className="text-[#2D2416] flex items-center gap-2">
                <User className="h-4 w-4 text-[#FF7A00]" />
                Login Sebagai
              </Label>
              <Select value={role} onValueChange={(value) => setRole(value as 'customer' | 'petani')}>
                <SelectTrigger className="mt-2 bg-[#FAF5EF] border-[#FF7A00]/20 hover:border-[#FF7A00]/40 transition-all duration-300">
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="animate-slide-in-up pt-2" style={{ animationDelay: '0.4s' }}>
              <Button 
                type="submit" 
            ...
            <div className="text-center mt-6 space-y-2">
              <p className="text-sm text-[#5A4A32]">
                Belum punya akun?{' '}
                <Link to="/register" className="text-[#FF7A00] hover:text-[#B82601] font-semibold">
                  Daftar Sekarang
                </Link>
              </p>
              <p className="text-xs text-[#5A4A32]/70">
                Admin?{' '}
                <Link to="/admin-login" className="text-[#B82601] hover:text-[#FF7A00] font-semibold underline">
                  Login sebagai Admin
                </Link>
              </p>
            </div>
